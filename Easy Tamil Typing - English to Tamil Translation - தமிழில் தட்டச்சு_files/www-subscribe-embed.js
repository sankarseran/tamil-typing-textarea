...
function Ed(a){for(var b=new vd(0,Infinity,Infinity,0),c=Ec(a),d=c.f.body,e=c.f.documentElement,f=Oc(c.f);a=Dd(a);)if(!(B&&0==a.clientWidth||Yb&&0==a.clientHeight&&a==d)&&a!=d&&a!=e&&"visible"!=Bd(a,"overflow")){var k=Fd(a),g=new D(a.clientLeft,a.clientTop);k.x+=g.x;k.y+=g.y;b.top=Math.max(b.top,k.y);b.right=Math.min(b.right,k.x+a.clientWidth);b.bottom=Math.min(b.bottom,k.y+a.clientHeight);b.left=Math.max(b.left,k.x)}d=f.scrollLeft;f=f.scrollTop;b.left=Math.max(b.left,d);b.top=Math.max(b.top,f);c=
...
a=ee(e.join(" "));a=[c,a];0==k.length||a.push(k.join(""));return a.join("_")}
...
function Ke(a,b){var c=b||{};"function"==typeof b&&(c={},c.callback=b);He(a,c);var d=a?a.split(":"):[],e=c.h||Ae(),f=I(J,"ah",Yd());if(f["::"]&&d.length){for(var k=[],g=null;g=d.shift();){var h=g.split(".");h=f[g]||f[h[1]&&"ns:"+h[0]||""]||e;var l=k.length&&k[k.length-1]||null,p=l;l&&l.hint==h||(p={hint:h,features:[]},k.push(p));p.features.push(g)}var q=k.length;if(1<q){var r=c.callback;r&&(c.callback=function(){0==--q&&r()})}for(;d=k.shift();)Le(d.features,c,d.hint)}else Le(d||[],c,e)}
function Le(a,b,c){function d(ea,xa){if(q)return 0;Sd.clearTimeout(p);r.push.apply(r,v);var jc=((Zd||{}).config||{}).update;jc?jc(f):f&&I(J,"cu",[]).push(f);if(xa){ie("me0",ea,t);try{Ie(xa,c,l)}finally{ie("me1",ea,t)}}return 1}
a=Xd(a)||[];var e=b.callback,f=b.config,k=b.timeout,g=b.ontimeout,h=b.onerror,l=void 0;"function"==typeof h&&(l=h);var p=null,q=!1;if(k&&!g||!k&&g)throw"Timeout requires both the timeout parameter and ontimeout parameter to be set";h=I(be(c),"r",[]).sort();var r=I(be(c),"L",[]).sort(),t=[].concat(h);0<k&&(p=Sd.setTimeout(function(){q=!0;g()},k));
var v=De(a,r);if(v.length){v=De(a,h);var Q=I(J,"CP",[]),F=Q.length;Q[F]=function(ea){function xa(){var Xc=Q[F+1];Xc&&Xc()}
function jc(Xc){Q[F]=null;d(v,ea)&&ae(function(){e&&e();Xc()})}
if(!ea)return 0;ie("ml1",v,t);0<F&&Q[F-1]?Q[F]=function(){jc(xa)}:jc(xa)};
if(v.length){var Wa="loaded_"+J.I++;Zd[Wa]=function(ea){Q[F](ea);Zd[Wa]=null};
a=se(c,v,"gapi."+Wa,h);h.push.apply(h,v);ie("ml0",v,t);b.sync||Sd.___gapisync?Fe(a):Ge(a)}else Q[F](Vd)}else d(v)&&e&&e()}
...
var If=function(){if(qc)return Hf(/Firefox\/([0-9.]+)/);if(B||Vb||Ub)return lc;if(uc)return Qb()?Hf(/CriOS\/([0-9.]+)/):Hf(/Chrome\/([0-9.]+)/);if(vc&&!Qb())return Hf(/Version\/([0-9.]+)/);if(rc||sc){var a=/Version\/(\S+).*Mobile\/(\S+)/.exec(yb);if(a)return a[1]+"."+a[2]}else if(tc)return(a=Hf(/Android\s+([0-9.]+)/))?a:Hf(/Version\/([0-9.]+)/);return""}();function Jf(a,b,c,d,e,f,k){var g;if(g=c.offsetParent){var h="HTML"==g.tagName||"BODY"==g.tagName;if(!h||"static"!=Bd(g,"position")){var l=Fd(g);if(!h){h=Ld(g);var p;if(p=h){if(p=vc)p=0<=sb(If,10);var q;if(q=ec)q=0<=sb(Gf,10);p=Xb||p||q}h=p?-g.scrollLeft:!h||Wb&&mc("8")||"visible"==Bd(g,"overflowX")?g.scrollLeft:g.scrollWidth-g.clientWidth-g.scrollLeft;l=Cc(l,new D(h,g.scrollTop))}}}g=l||new D;l=Kd(a);if(h=Ed(a)){var r=new wd(h.left,h.top,h.right-h.left,h.bottom-h.top);h=Math.max(l.left,r.left);p=
Math.min(l.left+l.width,r.left+r.width);h<=p&&(q=Math.max(l.top,r.top),r=Math.min(l.top+l.height,r.top+r.height),q<=r&&(l.left=h,l.top=q,l.width=p-h,l.height=r-q))}h=Ec(a);q=Ec(c);if(h.f!=q.f){p=h.f.body;q=Pc(q.f);r=new D(0,0);var t=(t=Gc(p))?Pc(t):window;b:{try{Rb(t.parent);var v=!0;break b}catch(Wa){}v=!1}if(v){v=p;do{var Q=t==q?Fd(v):Gd(v);r.x+=Q.x;r.y+=Q.y}while(t&&t!=q&&t!=t.parent&&(v=t.frameElement)&&(t=t.parent))}v=Cc(r,Fd(p));!B||9<=Number(pc)||Mc(h.f)||(v=Cc(v,Nc(h.f)));l.left+=v.x;l.top+=
v.y}a=Kf(a,b);b=l.left;a&4?b+=l.width:a&2&&(b+=l.width/2);b=new D(b,l.top+(a&1?l.height:0));b=Cc(b,g);e&&(b.x+=(a&4?-1:1)*e.x,b.y+=(a&1?-1:1)*e.y);var F;k&&(F=Ed(c))&&(F.top-=g.y,F.right-=g.x,F.bottom-=g.y,F.left-=g.x);return Lf(b,c,d,f,F,k,void 0)}
function Lf(a,b,c,d,e,f,k){a=a.clone();var g=Kf(b,c);c=Id(b);k=k?k.clone():c.clone();a=a.clone();k=k.clone();var h=0;if(d||0!=g)g&4?a.x-=k.width+(d?d.right:0):g&2?a.x-=k.width/2:d&&(a.x+=d.left),g&1?a.y-=k.height+(d?d.bottom:0):d&&(a.y+=d.top);if(f){if(e){d=a;g=k;h=0;65==(f&65)&&(d.x<e.left||d.x>=e.right)&&(f&=-2);132==(f&132)&&(d.y<e.top||d.y>=e.bottom)&&(f&=-5);d.x<e.left&&f&1&&(d.x=e.left,h|=1);if(f&16){var l=d.x;d.x<e.left&&(d.x=e.left,h|=4);d.x+g.width>e.right&&(g.width=Math.min(e.right-d.x,
...
256;h=e}f=new wd(0,0,0,0);f.left=a.x;f.top=a.y;f.width=k.width;f.height=k.height;e=h;if(e&496)return e;a=new D(f.left,f.top);a instanceof D?(k=a.x,a=a.y):(k=a,a=void 0);b.style.left=Hd(k,!1);b.style.top=Hd(a,!1);k=new Dc(f.width,f.height);c==k||c&&k&&c.width==k.width&&c.height==k.height||(c=k,a=Mc(Ec(Gc(b)).f),!B||mc("10")||a&&mc("8")?(b=b.style,Xb?b.MozBoxSizing="border-box":Yb?b.WebkitBoxSizing="border-box":b.boxSizing="border-box",b.width=Math.max(c.width,0)+"px",b.height=Math.max(c.height,0)+
...
d=Pd(b,"borderRight"),g=Pd(b,"borderTop"),b=Pd(b,"borderBottom"),b=new vd(g,d,b,f)),k.pixelWidth=c.width-b.left-a.left-a.right-b.right,k.pixelHeight=c.height-b.top-a.top-a.bottom-b.bottom):(k.pixelWidth=c.width,k.pixelHeight=c.height)));return e}
...
*/
var ug=window.yt&&window.yt.config_||window.ytcfg&&window.ytcfg.data_||{};x("yt.config_",ug);function vg(a){var b=arguments;1<b.length?ug[b[0]]=b[1]:1===b.length&&Object.assign(ug,b[0])}
...
;function Dh(a){a=void 0===a?{}:a;Ca(a)&&(a={callback:a});if(a.gapiHintOverride||O("GAPI_HINT_OVERRIDE")){var b=document.location.href;-1!=b.indexOf("?")?(b=(b||"").split("#")[0],b=b.split("?",2),b=Ah(1<b.length?b[1]:b[0])):b={};(b=b.gapi_jsh)&&jb(a,{_c:{jsl:{h:b}}})}Ke("gapi.iframes:gapi.iframes.style.common",a)}
...
function ni(a,b,c,d,e,f){mf(c,X(a,"tip-reverse"),f);var k=0;f&&(k=1);var g=Id(b);f=new D((g.width-10)/2,f?g.height:0);var h=Fd(b);Lf(new D(h.x+f.x,h.y+f.y),c,k);f=Lc(window);if(1==c.nodeType)var l=Gd(c);else c=c.changedTouches?c.changedTouches[0]:c,l=new D(c.clientX,c.clientY);c=Id(d);var p=Math.floor(c.width/2);k=!!(f.height<l.y+g.height);g=!!(l.y<g.height);h=!!(l.x<p);f=!!(f.width<l.x+p);l=(c.width+3)/-2- -5;a=a.j(b,"force-tooltip-direction");if("left"==a||h)l=-5;else if("right"==a||f)l=20-c.width-
...
function nj(a,b,c){c=void 0===c?{}:c;var d={"X-Goog-Visitor-Id":c.visitorData||O("VISITOR_DATA","")};if(b&&b.includes("www.youtube-nocookie.com"))return d;(b=c.Ob||O("AUTHORIZATION"))||(a?b="Bearer "+u("gapi.auth.getToken")().Nb:b=fe([]));b&&(d.Authorization=b,d["X-Goog-AuthUser"]=O("SESSION_INDEX",0),qi("pageid_as_header_web")&&(d["X-Goog-PageId"]=O("DELEGATED_SESSION_ID")));return d}
function oj(a){a=Object.assign({},a);delete a.Authorization;var b=fe();if(b){var c=new cf;c.update(O("INNERTUBE_API_KEY",void 0));c.update(b);b=c.digest();c=3;void 0===c&&(c=0);if(!xc){xc={};for(var d="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".split(""),e=["+/=","+/","-_=","-_.","-_"],f=0;5>f;f++){var k=d.concat(e[f].split(""));wc[f]=k;for(var g=0;g<k.length;g++){var h=k[g];void 0===xc[h]&&(xc[h]=g)}}}c=wc[c];d=[];for(e=0;e<b.length;e+=3){var l=b[e],p=(f=e+1<b.length)?b[e+1]:
...
function Uj(a,b,c,d){var e=H(b,X(a,"group")),f=!!a.j(b,"button-menu-ignore-group");e=e&&!f?e:b;f=9;var k=8,g=Kd(b);if(K(b,X(a,"reverse"))){f=8;k=9;g=g.top+"px";try{c.style.maxHeight=g}catch(p){}}K(b,"flip")&&(K(b,X(a,"reverse"))?(f=12,k=13):(f=13,k=12));var h;a.j(b,"button-has-sibling-menu")?h=Dd(e):a.j(b,"button-menu-root-container")&&(h=ak(a,b));B&&!mc("8")&&(h=null);if(h){var l=Kd(h);l=new vd(-l.top,l.left,l.top,-l.left)}h=new D(0,1);K(b,X(a,"center-menu"))&&(h.x-=Math.round((Id(c).width-Id(b).width)/
2));d&&(h.y+=Nc(document).y);if(a=Zj(a,b))b=Id(c),a.style.width=b.width+"px",a.style.height=b.height+"px",Jf(e,f,a,k,h,l,197),d&&xd(a,"position","fixed");Jf(e,f,c,k,h,l,197)}
...
this.v={};ff(this.l);this.l=null;nk.A.unregister.call(this)};
...
function sk(a,b,c){var d=tk(a,b);if(d){var e=Id(c);if(e instanceof Dc){var f=e.height;e=e.width}else throw Error("missing height argument");d.style.width=Hd(e,!0);d.style.height=Hd(f,!0)}c==a.f&&(e=9,f=8,K(b,X(a,"reversed"))&&(e^=1,f^=1),K(b,X(a,"flipped"))&&(e^=4,f^=4),a=new D(0,1),d&&Jf(b,e,d,f,a,null,197),Jf(b,e,c,f,a,null,197))}
...
dl.prototype.g=function(a){if(this.G){a={isSubscribed:a};try{this.G.send("msg-hovercard-subscription",a,void 0,u("gapi.iframes.SAME_ORIGIN_IFRAMES_FILTER"))}catch(b){}}};function gl(){Dh(function(){var a=Id(E("yt-subscribe"));a={width:a.width,height:a.height};var b=hl;Eh().ready(a,null,b)})}
